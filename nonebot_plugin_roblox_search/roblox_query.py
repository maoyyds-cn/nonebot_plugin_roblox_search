...
async def handle_search(event: Event):
    ...
    try:
        ...
        details_task = asyncio.create_task(get_user_details(user_id))
        status_task = asyncio.create_task(get_user_status(user_id))
        groups_task = asyncio.create_task(get_groups(user_id))
        friend_count_task = asyncio.create_task(get_friend_count(user_id))
        follower_count_task = asyncio.create_task(get_follower_count(user_id))
        following_count_task = asyncio.create_task(get_following_count(user_id))
        avatar_url_task = asyncio.create_task(get_avatar_url(user_id))
        headshot_url_task = asyncio.create_task(get_avatar_headshot_url(user_id))
        
        await asyncio.gather(
            details_task, status_task, groups_task,
            friend_count_task, follower_count_task,
        ...
        display_name = details.get("displayName", raw_name) if details else raw_name
        created = details.get("created", "") if details else ""
        ...
        await roblox_search.finish(messages)
        ...
        print("[查询错误]", traceback.format_exc())
        await roblox_search.finish(f"查询失败：{str(e)}")
    finally:
